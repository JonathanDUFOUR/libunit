...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2022/01/09 19:17:54 by jodufour          #+#    #+#             */
/*   Updated: 2022/01/09 21:52:33 by jodufour         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int	cvrt_c_s_test(void)
{
	...
	if (redirect_init())
		return (EXIT_FAILURE);
	d.ret0 = printf("%c something after", 'S');
	fflush(stdout);
	if (d.ret0 > 0)
	...
	d.ret1 = ft_printf("%c something after", 'S');
	if (d.ret1 > 0)
		d.rd = read(STDIN_FILENO, d.buff, 4096);
	while (d.rd)
	{
