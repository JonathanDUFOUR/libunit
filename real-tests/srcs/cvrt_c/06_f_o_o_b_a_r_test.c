...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2022/01/09 19:34:28 by jodufour          #+#    #+#             */
/*   Updated: 2022/01/09 21:52:57 by jodufour         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int	cvrt_c_f_o_o_b_a_r_test(void)
{
	...
	if (redirect_init())
		return (EXIT_FAILURE);
	d.ret0 = printf("%c%c%c%c%c%c", 'f', 'o', 'o', 'b', 'a', 'r');
	fflush(stdout);
	if (d.ret0 > 0)
	...
	d.ret1 = ft_printf("%c%c%c%c%c%c", 'f', 'o', 'o', 'b', 'a', 'r');
	if (d.ret1 > 0)
		d.rd = read(STDIN_FILENO, d.buff, 4096);
	while (d.rd)
	{
