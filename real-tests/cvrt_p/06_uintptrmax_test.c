...
/*   Updated: 2022/01/09 22:10:06 by jodufour         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int	cvrt_p_uintptrmax_test(void)
{
	...
	if (redirect_init())
		return (EXIT_FAILURE);
	d.ret0 = printf("%p is definitely my maximum.", (void *)__UINTPTR_MAX__);
	fflush(stdout);
	if (d.ret0 > 0)
	...
	d.ret1 = ft_printf("%p is definitely my maximum.", (void *)__UINTPTR_MAX__);
	if (d.ret1 > 0)
		d.rd = read(STDIN_FILENO, d.buff, 4096);
	while (d.rd)
	{
